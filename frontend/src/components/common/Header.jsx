            ...
            {isAuthenticated ? (
              <div className="header-user-menu">
                <button type="button" className="header-user-trigger" onClick={() => setIsUserMenuOpen((open) => !open)} aria-haspopup="menu" aria-expanded={isUserMenuOpen}>
                  ...
                  <span className="header-user-copy"><strong>{user.name}</strong><small>{ROLE_LABELS[user.role] || user.role}</small></span>
                  <ChevronDown size={15} aria-hidden="true" />
                </button>
                {isUserMenuOpen && <div className="user-dropdown" role="menu"><NavLink to="/profile" role="menuitem" onClick={() => setIsUserMenuOpen(false)}><UserRound size={16} />Profile</NavLink><NavLink to="/notifications" role="menuitem" onClick={() => setIsUserMenuOpen(false)}><Bell size={16} />Notifications</NavLink><button type="button" role="menuitem" onClick={() => setShowLogoutDialog(true)}><LogOut size={16} />Log out</button></div>}
              </div>
            ) : !isAuthPage && (
              <><NavLink to="/login" className="button button-secondary button-md header-login-button">Log in</NavLink><NavLink to="/register" className="button button-primary button-md header-cta">Get started</NavLink></>
            )}
          </div>
        </div>

        {showTopNavigation && isMobileMenuOpen && <nav className="mobile-marketing-menu" aria-label="Mobile navigation">{links.map((item, index) => <Link key={`${item.label}-${index}`} to={item.to} onClick={closeMobileMenu}>{item.label}</Link>)}{!isAuthenticated && <div className="mobile-auth-actions"><Link to="/login" onClick={closeMobileMenu} className="button button-secondary">Log in</Link><Link to="/register" onClick={closeMobileMenu} className="button button-primary">Get started</Link></div>}</nav>}
      </header>

      <ConfirmDialog isOpen={showLogoutDialog} title="Log out of KIT Support Hub?" message="You will need to enter your credentials again to access protected support pages." confirmLabel="Log out" isBusy={isLoggingOut} onCancel={() => setShowLogoutDialog(false)} onConfirm={handleLogout} />
    </>
  )
}
