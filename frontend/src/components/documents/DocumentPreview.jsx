...
export default function DocumentPreview({ document }) {
  ...
  if (query.isError)
    return (
      <ErrorState
        message="This preview is unavailable."
  ...
  if (
    ...
    return (
      <div className="grid min-h-80 place-items-center rounded-xl border border-dashed border-slate-300 bg-slate-50 p-8 text-center dark:border-zinc-700 dark:bg-zinc-950">
        <div>
          <FileQuestion className="mx-auto text-slate-400" size={38} />
  ...
  if (document.document_type === "image")
    return (
      <div className="flex min-h-[520px] items-center justify-center overflow-hidden rounded-xl border border-slate-200 bg-slate-100 p-4 dark:border-zinc-800 dark:bg-zinc-950">
        <img
  ...
  return (
    <iframe
      ...
      src={preview.preview_url}
      className="h-[70vh] min-h-[520px] w-full rounded-xl border border-slate-200 bg-white dark:border-zinc-800"
    />
  );
}
