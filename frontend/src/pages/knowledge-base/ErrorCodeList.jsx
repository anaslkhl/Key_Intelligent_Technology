...
export default function ErrorCodeList() {
  ...
  const products = useMemo(
    () =>
      (familiesQuery.data || [])
        ...
        .filter((product) => !familyId || product.family_id === familyId),
    [familiesQuery.data, familyId],
  );
  const query = useQuery({
    queryKey: ["error-codes", { search, familyId, productId, severity, page }],
    queryFn: () =>
      apiClient
        .get("/error-codes", {
          params: {
  ...
  const syncUrl = (overrides = {}) => {
    const values = {
      ...
      family_id: overrides.familyId ?? familyId,
      product_id: overrides.productId ?? productId,
  ...
  return (
    <SelfServicePage>
      ...
      <PageHeader
        ...
        title="Error Codes Library"
        description="Search and browse error codes for KIT Robotics products to quickly identify and resolve issues."
        actions={
      ...
      <form
        onSubmit={submit}
        className="mt-6 grid gap-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-zinc-800 dark:bg-[#111111] lg:grid-cols-[minmax(240px,1fr)_200px_220px_160px_auto]"
      >
        <label className="grid gap-1.5 text-sm font-semibold text-slate-700 dark:text-zinc-200">
          ...
          <span className="relative">
            ...
            <input
              ...
              placeholder="e.g. E-104, navigation"
            />
          </span>
        </label>
        <label className="grid gap-1.5 text-sm font-semibold text-slate-700 dark:text-zinc-200">
          ...
          <select
            ...
            onChange={(event) => {
              ...
              setFamilyId(value);
              setProductId("");
              syncUrl({ familyId: value, productId: "" });
              setPage(1);
            }}
        ...
        <label className="grid gap-1.5 text-sm font-semibold text-slate-700 dark:text-zinc-200">
          ...
          <select
            ...
            onChange={(event) => {
              ...
              syncUrl({ productId: event.target.value });
              setPage(1);
            }}
            className={inputClass}
          >
        ...
        <label className="grid gap-1.5 text-sm font-semibold text-slate-700 dark:text-zinc-200">
          ...
          <select
            ...
            onChange={(event) => {
              ...
              syncUrl({ severity: event.target.value });
              setPage(1);
            }}
            className={inputClass}
          >
      ...
      <div className="mt-6">
        {query.isLoading && <LoadingState label="Searching error codes..." />}
        {query.isError && (
          <ErrorState
            message="Unable to load the error-code library."
        ...
        {query.isSuccess && codes.length === 0 && (
          <EmptyState
            ...
            description="Try another code, product, or keyword."
          />
        )}
        {codes.length > 0 && (
          <>
            <div className="grid gap-4 lg:grid-cols-2">
              {codes.map((item) => (
                <ErrorCodeCard key={item.id} item={item} />
              ))}
            </div>
...
function ErrorCodeCard({ item }) {
  return (
    <article className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-[#111111]">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          <span className="grid h-10 w-10 place-items-center rounded-lg bg-blue-50 text-blue-600 dark:bg-blue-950/40">
            <Wrench size={19} />
          </span>
          <div>
        ...
        <span
          className={`rounded-full px-2.5 py-1 text-xs font-bold uppercase ${severityStyles[item.severity]}`}
        >
      ...
      <dl className="mt-4 grid gap-4 text-sm">
        <div>
          <dt className="font-semibold text-slate-800 dark:text-zinc-200">
            Likely cause
          </dt>
          <dd className="mt-1 leading-6 text-slate-600 dark:text-zinc-400">
            {item.cause}
          </dd>
        </div>
      ...
      {item.article && (
        <Link
          to={`/knowledge-base/${item.article.slug}`}
...
const inputClass =
  "h-11 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm text-slate-900 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-200 dark:border-zinc-700 dark:bg-[#111111] dark:text-white dark:focus:ring-blue-900";
