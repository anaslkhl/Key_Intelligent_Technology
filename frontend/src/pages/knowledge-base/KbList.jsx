...
export default function KbList() {
  ...
  const highlightsQuery = useQuery({
    queryKey: ["kb-highlights", isAuthenticated],
    queryFn: () =>
      apiClient
  ...
  const suggestionsQuery = useQuery({
    ...
    queryFn: () =>
      apiClient
        .get("/knowledge-base/suggest", { params: { q: debouncedSearch } })
        .then((response) => response.data.data),
  ...
  const query = useQuery({
    queryKey: [
      "knowledge-base",
      { search: debouncedSearch, familyId, category, page },
    ],
    queryFn: () =>
      apiClient
        .get("/knowledge-base", {
  ...
  const updateUrl = (next = {}) => {
    const values = {
      ...
      family_id: next.familyId ?? familyId,
    ...
    <SelfServicePage>
        ...
        <div className="relative">
          ...
          {suggestionsOpen && debouncedSearch.length >= 2 && (
            <div
              id="kb-suggestions"
              role="listbox"
              className="absolute z-30 mt-2 w-full overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xl dark:border-zinc-800 dark:bg-[#111111]"
            >
              ...
              {suggestions.map((suggestion, index) => (
                ...
                >
                  <span className="min-w-0 truncate font-semibold text-slate-800 dark:text-zinc-100">
                    {suggestion.title}
                  </span>
      ...
      {!debouncedSearch && !familyId && !category && (
        <div className="mt-8 grid gap-6">
          <HighlightShelf
            title="Most viewed"
            description="The guidance clients rely on most."
            icon={TrendingUp}
            articles={highlightsQuery.data?.most_viewed}
            isLoading={highlightsQuery.isLoading}
          />
          <HighlightShelf
            title="Most helpful"
            description="Articles with the most positive feedback."
            icon={Heart}
            articles={highlightsQuery.data?.most_helpful}
            isLoading={highlightsQuery.isLoading}
          />
          {isAuthenticated &&
            (highlightsQuery.isLoading ||
              highlightsQuery.data?.recently_viewed?.length > 0) && (
              <HighlightShelf
                title="Recently viewed"
                description="Pick up where you left off."
                icon={History}
                articles={highlightsQuery.data?.recently_viewed}
                isLoading={highlightsQuery.isLoading}
              />
            )}
        </div>
      ...
      <div className="mt-6">
        ...
        {query.isError && (
          <ErrorState
            message="Unable to load knowledge-base articles."
        ...
        {query.isSuccess && articles.length === 0 && (
          <EmptyState
            ...
            description="Try changing your search, category, or product-family filter."
          />
        )}
        {articles.length > 0 && (
          <>
      ...
      <div className="mt-3 grid auto-cols-[minmax(245px,1fr)] grid-flow-col gap-3 overflow-x-auto pb-2 xl:grid-flow-row xl:grid-cols-5 xl:overflow-visible">
        ...
        {articles.map((article, index) => (
          ...
          >
            ...
            <strong className="mt-2 line-clamp-2 text-sm text-slate-900 group-hover:text-blue-600 dark:text-white">
              {article.title}
            </strong>
            <span className="mt-auto flex items-center justify-between pt-3 text-xs text-slate-500">
    ...
    >
      ...
      {title && (
        <>
          <ChevronRight size={14} />
          <span
            className="max-w-md truncate font-semibold text-slate-800 dark:text-zinc-100"
            aria-current="page"
          >
            {title}
          </span>
