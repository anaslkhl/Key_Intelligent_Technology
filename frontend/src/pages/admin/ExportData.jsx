...
export default function ExportData() {
  ...
  const startDownload = async (type) => { if (await trigger()) download.mutate({ type, values: values() }) }
  ...
  return <AdminPage><PageHeader eyebrow="Administration" title="Export data" description="Download user and support-ticket records for reporting or analysis." />
    ...
    <div className="mt-6 grid gap-6 md:grid-cols-2"><ExportCard icon={Users} title="Users CSV" description="Name, email, role, account status, and creation date." busy={download.isPending} onClick={() => startDownload('users')} /><ExportCard icon={TicketCheck} title="Tickets CSV" description="Title, status, priority, client, robot, CSAT, and creation date." busy={download.isPending} onClick={() => startDownload('tickets')} /></div>
  </AdminPage>
}
